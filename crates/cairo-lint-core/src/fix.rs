...
use cairo_lang_syntax::node::ast::{
    BinaryOperator, BlockOrIf, Condition, ElseClause, Expr, ExprBinary, ExprIf, ExprLoop, ExprMatch, OptionElseClause,
...
use crate::lints::comparison_to_empty::is_empty_array;
...
pub fn fix_comparison_to_empty(db: &dyn SyntaxGroup, node: SyntaxNode) -> String {
    let expr: Expr = Expr::from_syntax_node(db, node.clone());
    ...
    if let Expr::Binary(binary_expr) = expr {
        ...
        if let BinaryOperator::Eq(_) | BinaryOperator::Neq(_) = operator {
            if is_empty_array(db, &lhs) {
                let rhs_text = rhs.as_syntax_node().get_text(db);
                return format!("{}.is_empty()", rhs_text);
            } else if is_empty_array(db, &rhs) {
                let lhs_text = lhs.as_syntax_node().get_text(db);
                return format!("{}.is_empty()", lhs_text);
            } else {
                panic!("Expected a comparison with an empty array, but found a different expression.");
            }
        } else {
            panic!("Unexpected binary operator. Only equality (==) and inequality (!=) are supported.");
        }
    }

    // Panic if the node is not a binary expression
    panic!("Expected a binary expression, but found something else.");
}
